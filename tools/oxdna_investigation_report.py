...
This script generates a focused report with purposeful figures that explain:
...
4. How this geometric verification can automate reward signals for training

Usage:
    conda activate cn24-agentic
    python -m tools.oxdna_investigation_report
"""
...
print("DEBUG: QT_QPA_PLATFORM set", flush=True)

print("DEBUG: importing json", flush=True)
import json
print("DEBUG: importing subprocess", flush=True)
import subprocess
print("DEBUG: importing numpy", flush=True)
import numpy as np
print("DEBUG: numpy imported", flush=True)

print("DEBUG: setting sys.path", flush=True)
...
def parse_oxdna_dat(dat_path):
    """Parse oxDNA .dat file → (coords, a1_vecs, a3_vecs) as np arrays."""
    coords, a1, a3 = [], [], []
...
def parse_oxdna_top(top_path):
    """Parse oxDNA .top file → (strand_ids, bases, n_strands)."""
    strands, bases = [], []
...
def pca_fit(coords):
    """PCA decomposition → (projected, eigenvalues, eigenvectors, pca_dims_nm)."""
...
def create_design_and_convert(n_helices=6, length=126):
    """Create flat sheet in cadnano, convert to oxDNA, return file paths."""
    ...
    return json_path, dat_path, top_path, app


# ============================================================
# FIGURE GENERATORS — each figure has a specific purpose
# ============================================================

def figure1_oxdna_format_anatomy(dat_path, top_path):
    """
    PURPOSE: Explain the oxDNA file format to someone unfamiliar with it.
    Shows what each column in the .dat file means and how the topology
    file maps nucleotides to strands.

    This is essential context: the oxDNA format is the bridge between
    cadnano's abstract connectivity graph and real 3D geometry.
    """
    import matplotlib
    matplotlib.use('Agg')
    ...
    # Column breakdown with color coding
    groups = [
        ('Position (x, y, z)', vals[0:3], '#2563eb', 'Center-of-mass of nucleotide\nin oxDNA units (1 unit = 0.8518 nm)'),
        ('Base normal (a1)', vals[3:6], '#dc2626', 'Points from backbone toward\nthe base-pairing face'),
        ('Stacking axis (a3)', vals[6:9], '#16a34a', "Points along the helix axis\n(5'→3' direction)"),
        ('Velocities (v, L)', vals[9:15] if len(vals) >= 15 else ['0']*6, '#9333ea', 'Linear + angular velocity\n(zero for static structures)'),
    ]
    ...
    ax.text(0.5, 0.9, 'b = 174.08 174.08 174.08 ← simulation box size', fontsize=8.5,
            fontfamily='monospace', color='#888')
    ax.text(0.5, 0.5, 'E = 0.00 0.00 0.00       ← energy (kinetic, potential, total)', fontsize=8.5,
            fontfamily='monospace', color='#888')
    ...
    # Show example lines with annotations
    y = 6.2
    colors_strand = ['#dc2626', '#dc2626', '#dc2626', '#dc2626',
    ...
    # Annotations
    ax.text(1, 2.8, 'Strand 1 (red): scaffold or staple #1', fontsize=9,
            color='#dc2626', fontweight='bold')
    ax.text(1, 2.3, 'Strand 2 (green): scaffold or staple #2', fontsize=9,
...
def figure2_conversion_pipeline(json_path, coords, strands):
    """
    PURPOSE: Show the conversion from cadnano JSON (abstract connectivity)
    to oxDNA 3D coordinates, demonstrating that tacoxDNA correctly maps
    the abstract design into physical space.

    This answers the question: "Does the cadnano → oxDNA conversion
    produce geometrically sensible output?"
    """
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt

    # Load cadnano JSON to show structure
    ...
    # Draw helices as horizontal lines with connectivity
    for i, vs in enumerate(vstrands):
        ...
        if occupied:
            ...
            # Mark crossovers (where next_vh != current helix)
            for j in occupied:
                next_vh, next_idx = scaf[j][2], scaf[j][3]
    ...
    # Show PCA-aligned bounding box as rectangle overlay
    ax3.scatter(proj_nm[:, 0], proj_nm[:, 1], c=c, s=2, alpha=0.3)

    # Draw PCA bounding box
...
def figure3_planarity_and_helix_structure(coords, strands, a1_vecs, a3_vecs):
    """
    PURPOSE: Demonstrate that the oxDNA output captures the physical
    structure of DNA helices — the double-helix geometry, inter-helix
    spacing, and sheet planarity are all visible in the 3D coordinates.

    This validates that tacoxDNA produces physically meaningful output
    that can be used for geometric reward signals.
    """
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    ...
    # --- Panel 1: Side view showing helix thickness ---
    ax = axes[0]
    ...
    # --- Panel 2: Cross-section with helix identification ---
    ax = axes[1]
    ax.set_title('Cross-Section: Individual Helices\n(PC2 vs PC3, averaged along PC1)',
                 fontsize=11, fontweight='bold')

    # Group nucleotides by their approximate PC2 position (helix identity)
    cross_section = proj_nm[:, 1:3]

    # K-means-like: find helix centers by binning PC2
    from scipy.cluster.hierarchy import fcluster, linkage
    linkage_matrix = linkage(cross_section, method='ward')
    # We know there are 6 helices worth of cross-section clusters
    n_clusters = 6
...
def figure4_training_integration(coords, strands):
    """
    PURPOSE: Show how geometric verification maps to a reward signal
    for training. This is the key figure that connects oxDNA conversion
    to the agentic training pipeline.

    Demonstrates: given a target geometry (flat sheet), the PCA dimensions
    and flatness ratio provide a continuous, differentiable reward signal
    that an RL agent can optimize against.
    """
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    ...
    # --- Panel 1: Reward function landscape ---
    ax = axes[0]
    ax.set_title('Flatness Ratio → Reward\n(How training rewards geometric correctness)',
                 fontsize=11, fontweight='bold')

    # Simulate a reward curve: flatness_ratio → reward
    # For a flat sheet target: reward = 1 - flatness_ratio (lower flatness = better)
    flatness_values = np.linspace(0, 0.5, 100)
    # Sigmoid-shaped reward: sharp penalty for non-flat structures
    ...
    # Mark where our design falls
    projected, _, _, pca_dims = pca_fit(coords)
    sorted_dims = np.sort(pca_dims)
    our_flatness = sorted_dims[0] / sorted_dims[2]
    ...
    # --- Panel 2: Multi-metric reward decomposition ---
    ax = axes[1]
    ax.set_title('Reward Decomposition\n(Multiple geometric metrics contribute to final score)',
                 fontsize=11, fontweight='bold')

    # Show how different metrics combine
    metrics = {
        'Flatness\n(thickness/length)': {'value': our_flatness, 'target': 0.0,
                                          'score': max(0, 1 - our_flatness / 0.15)},
        'Aspect ratio\n(length/width)': {'value': pca_dims[0] / pca_dims[1],
                                          'target': 4.0,
                                          'score': max(0, 1 - abs(pca_dims[0] / pca_dims[1] - 4.0) / 4.0)},
        'Planarity\nRMS (nm)': {'value': np.sqrt(np.mean((projected[:, 2] * OXDNA_UNIT_NM)**2)),
                                 'target': 0.0,
                                 'score': max(0, 1 - np.sqrt(np.mean((projected[:, 2] * OXDNA_UNIT_NM)**2)) / 2.0)},
        'Fill fraction\n(hull/bbox)': {'value': 0.75, 'target': 1.0,
                                        'score': 0.75},
    }
    ...
    for i, (bar, score, val) in enumerate(zip(bars, scores, values)):
        ...
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() / 2,
                f'val={val:.2f}', ha='center', fontsize=8, color='white',
                fontweight='bold')
...
def write_report(json_path, dat_path, top_path, coords, strands, a3_vecs, figure_paths):
    ...
    report = f"""# Investigation Report: cadnano → oxDNA Conversion for Automated Training
...
![Figure 1: oxDNA Format Anatomy]({os.path.relpath(figure_paths[0], REPORT_DIR)})
...
For cadnano → oxDNA conversion, it:
...
![Figure 2: Conversion Pipeline]({os.path.relpath(figure_paths[1], REPORT_DIR)})
...
![Figure 3: Planarity and Structure]({os.path.relpath(figure_paths[2], REPORT_DIR)})
...
1. **Target geometry specification:** "Create a flat sheet" →
   target flatness ≈ 0, target aspect ratio ≈ 4.0
...
![Figure 4: Training Integration]({os.path.relpath(figure_paths[3], REPORT_DIR)})
...
1. **tacoxDNA reliably converts cadnano designs to 3D coordinates.** The
   6-helix flat sheet produces a {pca_dims[0]:.1f} × {pca_dims[1]:.1f} × {pca_dims[2]:.1f} nm
...
def main():
    ...
    # Step 1: Create design and convert
    print("\n[1/6] Creating flat sheet design and converting to oxDNA...")
    json_path, dat_path, top_path, app = create_design_and_convert()

    # Step 2: Parse
    print("\n[2/6] Parsing oxDNA files...")
    ...
    # Step 3-6: Generate figures
    print("\n[3/6] Figure 1: oxDNA format anatomy...")
    fig1 = figure1_oxdna_format_anatomy(dat_path, top_path)

    print("\n[4/6] Figure 2: Conversion pipeline...")
    fig2 = figure2_conversion_pipeline(json_path, coords, strands)

    print("\n[5/6] Figure 3: Planarity and helix structure...")
    fig3 = figure3_planarity_and_helix_structure(coords, strands, a1_vecs, a3_vecs)

    print("\n[6/6] Figure 4: Training integration...")
    fig4 = figure4_training_integration(coords, strands)

    # Write report
    print("\nWriting report...")
    figure_paths = [fig1, fig2, fig3, fig4]
    report_path = write_report(json_path, dat_path, top_path, coords, strands, a3_vecs, figure_paths)
