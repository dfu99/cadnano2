...
We thus replaced the tool-calling architecture with a coding agent (Claude Code) that reads caDNAno's source code directly and writes Python scripts (@fig1, B). Source code access provided two capabilities absent from the tool-calling approach.
...
Additional errors in this phase included scaffold/staple strand assignment (the agent placed scaffold strands on the staple strand set and vice versa), which the user identified by inspecting the design in caDNAno's strand routing view.
...
With cavity routing understood, the agent developed a 4-step process for resizing and re-aligning the cavity (@fig4, C). After each step, the agent verified that the resulting JSON opened correctly in caDNAno without errors.
...
+ *Invisible in source code.* Rules such as the grid-to-layer mapping, the midseam routing pattern, and the crossover directionality conventions at cavity boundaries cannot be deduced from caDNAno's codebase. They require explicit transfer from an experienced designer.
...
The pipeline from the user's template to verified design to oxDNA simulation is end-to-end, requiring no manual caDNAno GUI interaction. For example, a researcher can describe a cavity design in terms of physical dimensions (nm #sym.times nm, scaffold length, padding) and receive a validated JSON, stapled design, and oxDNA files ready for submission. Integration with cluster scheduling can extend this further, including 3D analysis functions that automatically infer and flag structural properties from molecular dynamics simulation trajectories.
