...
- *Autonomous geometric verification.* A coding agent does not have geometric awareness of the designed DNA nanostructure; cadnano's 2D path view provides no spatial information about the 3D conformation. However, with the end-to-end pipeline in place, the agent could convert candidate designs to 3D coordinates via tacoxDNA and analyze the resulting structured file format. The agent used this capability to determine correct helix placement on the honeycomb lattice without human instruction: it generated candidate grid layouts, converted each to oxDNA format, computed cross-sectional slices by PCA decomposition of the 3D nucleotide coordinates, and evaluated whether the resulting profile matched a flat rectangle. By iterating over column arrangements and measuring cross-sectional circularity (flat sheet: aspect ratio #sym.approx 0.05; incorrect 2#sym.times 3 grid: #sym.approx 0.84), the agent converged on the correct layout (@fig2, b). This process, integration of a 3D simulation tool to compensate for geometric information absent from the design tool, was not anticipated in the original system design.
