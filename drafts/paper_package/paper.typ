...
Modifying a DNA origami design requires re-routing the scaffold, re-placing staples, and re-verifying the structure for each parameter change #cite(<majikes2021>) #cite(<aksel2018>). A single redesign of a simple structure may be quick, taking several minutes, but multiple iterations or greater complexity accumulate to hours of repetitive, error-prone work. The knowledge required to correctly perform these tasks is well documented for human practitioners #cite(<majikes2021>), but is not encoded in the design tools themselves. The tools implement the operations, but the rules governing when and how to compose those operations correctly remain in tutorials, lab conventions, and individual expertise. A coding agent can bridge this gap. It can read the tools' source code to learn what operations are available, acquire the composition rules through interaction with a designer, and produce executable artifacts that encode both together. The result is that a designer's expertise, once transferred, becomes a reproducible workflow that any subsequent user or agent can apply without re-learning the rules from scratch.

The effort to algorithmically accelerate the design process and eliminate design tedium has been approached from various angles. A substantial ecosystem of automated design tools now exists, spanning lattice-based design #cite(<douglas2009>), automated staple breaking #cite(<aksel2018>), curved and tubular structures #cite(<fu2024>), wireframe origami from 2D and 3D meshes #cite(<veneziano2016>) #cite(<benson2015>) #cite(<jun2019>) #cite(<jun2021>), shape annealing #cite(<babatunde2021>), and interactive 3D editing environments #cite(<levy2021>). However, there is little continuity between these tools. Each addresses a specific shape class or design subtask, embodying its own design principles and operating through a disconnected interface. In practice, the adoption friction of learning each new tool means that many designers default to a single familiar environment, even when a more specialized tool exists for their task. A coding agent that can read any open-source tool's source code could serve as a natural language interface across all of them, allowing designers to adopt new methodologies without learning each tool's interface individually. In this work, we initiate that investigation by retracing the DNA origami design chronology #cite(<rothemund2006>) #cite(<douglas2009_3d>) #cite(<ke2009>) #cite(<dietz2009>) #cite(<ke2012>), from the beginning; folding simple single and multi-layer designs, and developing the methodology for an agent-mediated pipeline that consolidates design principles, techniques, and tools toward greater structural complexity.

A coding agent is a large language model (LLM) that operates by reading files, writing and executing code, and iterating on the results within a command-line environment. Unlike chatbot-style LLMs that produce only text responses, coding agents take actions. They can navigate a codebase, call APIs, run scripts, inspect outputs, and modify files across multiple steps without human intervention between steps. Recent advances in LLM reasoning and tool use have made these agents capable of sustained, multi-step software engineering tasks #cite(<he2025>) #cite(<dong2025>), including navigating unfamiliar codebases, integrating independently developed software packages, and writing domain-specific scripts from documentation and source code alone. However, LLMs have no geometric understanding. They cannot reason about nucleotide positions in 3D space. cadnano2 provides a layer of abstraction that makes this unnecessary. It encodes helix connectivity, crossover placement, and strand routing as structured data rather than spatial coordinates, reducing DNA origami design to operations on lists and indices. This abstraction makes cadnano2 a tractable conduit for investigating whether a coding agent can acquire the domain knowledge needed to manipulate DNA origami designs through direct interaction with the software, rather than with the 3D structure itself.

In the current work, we demonstrate how a coding agent with direct access to cadnano2's source code can read internal data models, write and execute Python scripts, and integrate external tools, like tacoxDNA #cite(<suma2019>) and oxDNA #cite(<ouldridge2011>) #cite(<sulc2012>) #cite(<snodin2015>) #cite(<rovigatti2015>), into an end-to-end pipeline. Critically, the methodology that emerges from this process is not ephemeral. It is saved as verifier functions, parametric scripts, and documented failure catalogs, all of which are transferable, repeatable text-file instructions interpretable by another coding agent. This converts _ad hoc_ design tasks into repeatable, iterable procedures and provides a mechanism for converting established design principles #cite(<majikes2021>) into machine-executable code.

As a proof of concept, we demonstrate the construction of a parametric pipeline for a 2-layer rectangular origami with a tunable cavity using a coding agent (Claude Code). Achieving functional usage required iterating through several distinct failure modes, each corresponding to domain knowledge invisible in the source code (e.g., honeycomb grid-to-layer mapping, crossover parity rules, cavity orientation conventions). Each failure was resolved by explicit correction from the human designer, after which the agent did not repeat that error class. Once this knowledge was encoded, the agent autonomously extended the base template to arbitrary lattice dimensions, generated designs to physical specifications, executed targeted structural edits from natural language prompts, and ran the full pipeline from design through stapling to oxDNA molecular dynamics simulation.

#figure(
  image("figures/fig1_architecture.png", width: 90%),
  caption: [Architecture comparison. (A) Embedded tool-using LLM achieved 0% success on multi-step design tasks because the model could not resolve contextual constraints (helix parity, crossover directionality) from tool schemas alone. (B) Coding agent with source code access produced end-to-end verified designs by reading cadnano2 internals, writing Python scripts, and integrating external simulation tools. (C) Knowledge formalization workflow. The user specifies a design target; the agent writes and executes scripts; the user evaluates the result. On failure, the user identifies the failure mode and provides a correction, which the agent formalizes as pipeline scripts, verifier checks, and persistent lessons. The agent retries with this formalized knowledge until the design is correct. On success, working artifacts pass through verification to become persistent, distributable outputs: parametric pipelines, an automated design verifier, a failure catalog, and text-file instructions interpretable by any agent.],
) <fig1>
...
- *End-to-end pipeline construction.* The agent integrated cadnano2 with tacoxDNA (format conversion) and oxDNA (molecular dynamics) into a single automated pipeline. Each tool is open-source and individually straightforward, but the manual workflow is disjointed, requiring the user to navigate between separate interfaces, file formats, and documentation. The agent automated this by reading each tool's source code, resolving format requirements, and writing wrapper scripts.

However, source code access alone was not sufficient. Building a correct DNA origami design required domain knowledge that is not encoded in the codebase, such as lattice geometry conventions, crossover placement rules, and scaffold routing patterns. Acquiring this knowledge required iterative human correction. The following sections describe categories of domain knowledge the agent could not derive from code, the process by which each was resolved, and the artifacts that formalized each correction.

== 4. Formalizing Helix Placement on the Honeycomb Lattice

During the iterative correction process, the agent could produce outputs with incorrect lattice dimensions, fragmented scaffold routing, and disconnected strands (@fig2, A). A cold-start agent given only the instruction to build a 2-layer rectangle can produce a reasonable first attempt, but as additional domain concepts were introduced (half-crossovers, full crossovers, midseam patterns), the agent's understanding appeared to drift before stabilizing through formalization. The resolution in each case was the same. The user provided an explicit correction, and the agent encoded the rule as a persistent constraint.

=== 4.1 The "Layer" Ambiguity

One obstacle that arose during the correction process was the relationship between grid rows and physical layers on the honeycomb lattice. Helices are arranged in a hexagonal packing where adjacent rows are offset vertically. A single row of hexagonally packed helices occupies two distinct y-coordinates, because the vertices of a hexagon do not lie on a single horizontal line. During iterative teaching, the agent at one point constructed a 4-row structure when instructed to build a "2-layer" rectangle, interpreting the two y-coordinates as two physical layers. A 2-layer structure actually corresponds to 2 grid rows on the honeycomb lattice, because one physical layer comprises helices at both y-coordinates of a single hexagonal row.
...
With the layer definition resolved, the agent independently developed geometric verification through the end-to-end pipeline (@fig2, C). cadnano2's 2D strand routing view provides no spatial information about the 3D conformation of a design. However, with tacoxDNA integrated, the agent could convert candidate designs to 3D coordinates and analyze the resulting geometry. When prompted to verify that a design was physically "flat," the agent independently devised a verification pipeline. It generated candidate grid layouts, converted each to oxDNA format, computed cross-sectional slices by PCA decomposition of the 3D nucleotide coordinates, and evaluated whether the resulting profile matched a flat rectangle. By measuring cross-sectional circularity (flat sheet: aspect ratio #sym.approx 0.05; incorrect 2-by-3 grid #sym.approx 0.84), the agent could reject wrong helix arrangements without human feedback. This behavior was entirely unprompted. The agent was asked to verify flatness, not told how to do so, and independently identified the 3D conversion tools as a means to answer a question that the 2D design interface could not.
...
Additional errors in this phase included scaffold/staple strand assignment (the agent placed scaffold strands on the staple strand set and vice versa), which the user identified by inspecting the design in cadnano2's strand routing view.

=== 5.3 Generalization

After learning the midseam and half-crossover placement rules, the agent generalized to various 2-layer lattice dimensions without further instruction (@fig3, C), each with a single contiguous scaffold loop. The first of these served as the end-to-end validation of the pipeline from design through molecular dynamics simulation.

#figure(
  image("figures/fig3_scaffold_routing.png", width: 100%),
  caption: [Formalizing scaffold routing. (A) Early agent attempts. Top: the agent explores the path design space, placing strands at various lengths across helices. Bottom: after being told about full and half crossovers, the agent attempts to place them but has no knowledge of where they should go to produce a single continuous scaffold. (B) User-provided 2-by-6 rectangle template with correct crossover routing and a midseam that routes the scaffold in 1 continuous loop. The half-crossover vs full-crossover distinction and the midseam pattern were explained alongside this template. (C) After learning the midseam and half-crossover placement rules from (@fig3, B), the agent generalizes to a 2-by-8 rectangle with correct routing, a single contiguous scaffold loop, without further instruction.],
) <fig3>

== 6. Formalizing Cavity Design

With scaffold routing established for simple rectangles, we introduced a more complex design goal, a 2-layer rectangle with a rectangular cavity. The cavity is a gap in the interior of the structure along the helix length axis. We enforced the constraint that the total scaffold length could not exceed 8,064 nt (the p8064 scaffold). This introduced a second set of failure modes.
...
With cavity routing understood, the agent developed a 4-step process for resizing and re-aligning the cavity (@fig4, C). After each step, the agent verified that the resulting JSON opened correctly in cadnano2 without errors.
  + Remove staples (keep scaffold only).
  + Extend helix arrays and shift right-side crossovers.
  + Move midseam crossovers to center.
  + Expand cavity by moving boundary crossovers and re-aligning to the center.

Additional failures arose during scaling: the agent's script left stray staple entries with broken crossover references, causing cadnano2 to crash; the agent filled the cavity gap with continuous scaffold, destroying the cavity; and the scaling logic moved crossover positions independently per helix, creating skewed crossovers where both sides must share the same index. Each was resolved by user correction, and the agent did not repeat any of these error classes.
...
+ *Extended the template to arbitrary lattice dimensions* (e.g., 2-by-16, 2-by-20) by adding column pairs to both sides of the base template, maintaining a single contiguous scaffold loop at each step.
...
+ *Invisible in source code.* Rules such as the grid-to-layer mapping, the midseam routing pattern, and the crossover directionality conventions at cavity boundaries cannot be deduced from cadnano2's codebase. They require explicit transfer from an experienced designer.
...
+ *Distributable.* The verifier (`cadnano_verifier.py`) checks all documented failure modes automatically. The failure catalog (`failure_analysis.md`) documents each with symptom, root cause, fix, and verifier check. A new user or agent can apply these checks without repeating the original debugging.

The agent functions not as a designer but as a substrate for accumulating and distributing design knowledge (@fig1, C). Each user-agent interaction produces reusable verification and pipeline tools in addition to the design itself. @artifacts_table lists the artifacts from this work that are distributable to any DNA origami researcher working with cadnano2.
...
#figure(
  ...
  table(
    ...
    [`cadnano_verifier.py`], [Pre-flight design validation], [Run on any cadnano2 JSON before committing to synthesis],
...
The pipeline from the user's template to verified design to oxDNA simulation is end-to-end, requiring no manual cadnano2 GUI interaction. For example, a researcher can describe a cavity design in terms of physical dimensions (nm #sym.times nm, scaffold length, padding) and receive a validated JSON, stapled design, and oxDNA files ready for submission. Integration with cluster scheduling can extend this further, including 3D analysis functions that automatically infer and flag structural properties from molecular dynamics simulation trajectories.

The transition from demonstration to knowledge transfer occurred when the agent began generating verification tools from its own failure modes. The verifier was not designed top-down. It emerged from specific debugging sessions, each producing a check that prevents the corresponding failure for any subsequent user.

Returning to the question posed in the introduction, whether a coding agent could serve as a unified front end for DNA origami design, the answer from this initial investigation is qualified. The agent cannot replace domain expertise, but it can serve as a mechanism for capturing that expertise in executable form. The design rules formalized in this work began as corrections delivered in natural language and ended as automated scripts, verifier checks, and documented constraints that any agent or user can apply. This suggests a model in which each new design challenge, once resolved through agent-designer interaction, permanently reduces the friction of that task class for all subsequent users. Whether this model scales to the full diversity of DNA origami structures, like curved, wireframe, or complex 3D structures, remains to be demonstrated, but this work sets a starting point for investigating coding agents as a consolidating control surface for DNA origami design, one that folds in and combines existing and emerging design methodologies into a streamlined pipeline.
